...
func (c *ContractCaller) SendStateReceive(id *big.Int, data []byte, stateReceiveAddress common.Address, stateReceiveInstance *statereceiver.Statereceiver) (er error) {
	msg, err := c.StateReceiverABI.Pack("onStateReceive", id, data)
	if err != nil {
	...
	tx, err := stateReceiveInstance.OnStateReceive(auth, id, data)
	...
	Logger.Info("Sending state to childchain successfully", "txHash", tx.Hash().String())
